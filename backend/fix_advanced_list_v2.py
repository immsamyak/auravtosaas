...
def get_table_fields(model):
    fields = []
    # Add ForeignKey and OneToOneField
...
for app_config in apps.get_app_configs():
    ...
    for model in app_config.get_models():
        ...
        # Build headers
        # We replace "ID" with "Record" for the first column
        ...
        for i, f in enumerate(table_fields):
            if isinstance(f, models.BooleanField):
                ...
                # Includes Foreign Keys, CharFields, etc.
                        ...
                        <div x-show="open" 
                            ...
                            <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white/95 backdrop-blur z-10">
                                <div>
                                    ...
                                    <p class="text-slate-500 text-sm mt-1">{model_name.capitalize()} • ID: #{{{{ obj.pk }}}}</p>
                                </div>
        ...
        with open(list_html_path, 'w') as f:
            f.write(content)
        print(f"Fixed Datatable and Modal for {model_name}")

print("All Datatables have been dynamically fixed.")
