...
def get_table_fields(model):
    ...
    for f in model._meta.get_fields():
        if isinstance(f, (models.CharField, models.EmailField, models.BooleanField, models.IntegerField, models.DateTimeField, models.DecimalField, models.ImageField, models.FileField)):
...
def get_all_fields(model):
    fields = []
    for f in model._meta.get_fields():
        if isinstance(f, (models.CharField, models.TextField, models.EmailField, models.BooleanField, models.IntegerField, models.DateTimeField, models.DecimalField, models.ImageField, models.FileField, models.URLField)):
            if f.name in ['password', 'is_superuser', 'is_staff']:
                continue
            fields.append(f)
    return fields
...
for app_config in apps.get_app_configs():
    ...
    for model in app_config.get_models():
        ...
        # Build headers
        headers_html = '<th class="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">ID</th>\n'
        ...
        # We wrap the <tr> in x-data="{ open: false }" so the modal is scoped to the row
        ...
        for i, f in enumerate(table_fields):
            if isinstance(f, models.BooleanField):
                cells_html += f'''                    <td class="p-4">
                        ...
                        {{% else %}}
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700">Inactive</span>
                        {{% endif %}}
                    </td>\n'''
            elif isinstance(f, (models.ImageField, models.FileField)):
                cells_html += f'''                    <td class="p-4">
                        {{% if obj.{f.name} %}}
                            <img src="{{{{ obj.{f.name}.url }}}}" class="h-10 w-10 object-cover rounded-lg border border-slate-200" alt="img" onerror="this.style.display='none'">
                        {{% else %}}
                            <div class="h-10 w-10 bg-slate-100 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 text-xs">N/A</div>
                        {{% endif %}}
                    </td>\n'''
            else:
                if i == 0:
        ...
        modal_html = f'''
                    ...
                    <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center" x-cloak>
                        ...
                        <div x-show="open" 
                             ...
                             class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-y-auto m-4 z-10 relative flex flex-col">
                            ...
                            <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white/95 backdrop-blur z-10">
                                <div>
                                    <h3 class="text-2xl font-extrabold text-slate-900">{model_name.capitalize()} Details</h3>
                                    <p class="text-slate-500 text-sm mt-1">ID: #{{{{ obj.pk }}}}</p>
                                </div>
        ...
        for f in all_fields:
            if isinstance(f, models.BooleanField):
                ...
                modal_html += f'''                                <div class="col-span-1 md:col-span-2">
                                    ...
                                    {{% if obj.{f.name} %}}
                                        <img src="{{{{ obj.{f.name}.url }}}}" class="h-32 rounded-xl border border-slate-200 object-contain bg-slate-50 p-2" onerror="this.style.display='none'">
                                    {{% else %}}
        ...
        modal_html += f'''                            </div>
                            ...
                            <div class="px-8 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 rounded-b-2xl">
        ...
        with open(list_html_path, 'w') as f:
            f.write(content)
        print(f"Upgraded Datatable for {model_name}")

print("All Datatables have been dynamically upgraded.")
