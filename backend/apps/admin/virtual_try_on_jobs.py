...
class VirtualTryOnListView(PlatformAdminRequiredMixin, SearchFilterMixin, ListView):
    model = VirtualTryOn
...
class VirtualTryOnCreateView(PlatformAdminRequiredMixin, TailwindFormViewMixin, CreateView):
    model = VirtualTryOn
    template_name = 'admin/fitting/virtualtryon/form.html'
    fields = '__all__'
    success_url = reverse_lazy('admin:virtualtryon_list')

class VirtualTryOnDetailView(PlatformAdminRequiredMixin, DetailView):
    model = VirtualTryOn
