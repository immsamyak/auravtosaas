...
class SubscriptionPlanListView(SuperUserRequiredMixin, SearchFilterMixin, ListView):
...
class SubscriptionPlanCreateView(SuperUserRequiredMixin, TailwindFormViewMixin, CreateView):
    model = SubscriptionPlan
    template_name = 'admin/billing/subscriptionplan/form.html'
    fields = '__all__'
    success_url = reverse_lazy('admin:subscriptionplan_list')

class SubscriptionPlanUpdateView(SuperUserRequiredMixin, TailwindFormViewMixin, UpdateView):
    model = SubscriptionPlan
    template_name = 'admin/billing/subscriptionplan/form.html'
    fields = '__all__'
    success_url = reverse_lazy('admin:subscriptionplan_list')
